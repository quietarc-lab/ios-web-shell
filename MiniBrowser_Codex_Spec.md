...
同スレ連続、または最終試行の連続投稿制限ではAPのみを再接続する。AP完了後のIP確認で不変だった場合は、同じ世代のまま1秒間隔で追加2回までAPを再試行し、合計3回とも不変なら停止する。IP確認失敗やショートカット失敗は再試行せず停止する。複数スレモードでは投稿完了マーカー受信後、次スレへの遷移を1秒後に開始する（スキップ対象は即時）。
