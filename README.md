...
- The catalog header's non-persistent `複数スレ` toggle snapshots only catalog threads with no viewing history (up to 60, or fewer when necessary) and posts the in-memory draft once per thread in that fixed order. It waits one second between successful targets, rotates to a fresh UA generation after every two accepted threads, reuses the UA within each two-thread batch unless a known image/access restriction requires a handoff, gives a short continuous-post restriction one UA handoff per target and then skips only that target if it recurs, skips only 1,000-reply or unavailable threads for six hours, and performs one end-of-catalog refresh for unprocessed targets
