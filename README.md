...
- If a multi-thread post body references an isolated thread, the bounded recovery path opens that thread and watches the DOM for up to five minutes for a valid board-B replacement URL preceded by a `次` marker, including explanatory forms ending in `次` such as `隔離されたから次`, quoted markers, and a same-line marker. The monitor also checks `textContent` so compacted or hidden replies are not excluded. It replaces only the matching source URL in the in-memory comment, captures the replacement thread's starter image, clears open/read history, and starts a fresh multi-thread session. No post is attempted on the isolated thread; the moderation-stop alert and alarm remain until acknowledged. Device confirmation remains pending
