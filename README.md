...
- The catalog header's non-persistent `複数スレ` toggle snapshots up to 60 visible threads and posts the in-memory draft once per thread in that fixed order. It waits three seconds between targets, reuses the same UA unless a known image/access restriction requires a handoff, skips unavailable threads for six hours, and performs one end-of-catalog refresh for unprocessed targets
