...
- If a multi-thread post body references an isolated thread, the bounded recovery path opens that thread and accepts a replacement only when a valid board-B thread URL is rendered on the line immediately after a standalone `次` line (optionally quoted as `> 次`). It replaces only the matching source URL in the in-memory comment, captures the replacement thread's starter image, clears open/read history, and starts a fresh multi-thread session. No post is attempted on the isolated thread; the moderation-stop alert and alarm remain until acknowledged. Device confirmation remains pending
