...
- Automatic AP diagnostics record only the Shortcut open result, bounded callback status/error presence, AP purpose, scene/AP context, and any multi-thread stop-request source. A monitor cancellation (`NSURLErrorDomain` `-999`) is logged as a benign cancellation instead of a monitor failure; callback watchdog expiry remains a bounded communication stop. No callback payload, IP address, URL, Cookie, or page data is logged
...
- If a multi-thread post body references an isolated thread, the posting session stops without showing the moderation alert or starting its alarm while recovery is possible. The same WebView watches the isolated source for up to five active minutes, reloading every ten seconds, for a valid board-B replacement URL immediately before or after a line containing `次`, `つぎ`, or ASCII `next` in any case, including explanatory forms such as `隔離されたから次`, quoted markers, and same-line marker/URL forms. In parallel, a native URLSession posts the fixed Shift-JIS search keyword `定型 ・ ・` to Futaba search immediately and every 30 seconds; its grouped parent-thread IDs are merged with direct candidates and the smallest valid ID above the isolated source is tried first. Search failures and invalid candidates continue to the next cycle or candidate. When found, every matching source URL in the in-memory comment is replaced, the replacement starter image is captured into the existing handwriting store, open/read history is cleared, the catalog is refreshed from momentum order, and a fresh multi-thread session starts. Recovery success is non-modal; only recovery failure shows `隔離スレの次スレ復旧に失敗したため自動投稿を停止しました` with the existing confirmation-required alarm. Device confirmation remains pending
...
- Automatic-post diagnostics also identify AP Shortcut open/callback outcomes, AP completion scene decisions, and where a multi-thread stop request was enforced. These records are for post-run diagnosis only; the iOS/Shortcuts-owned data-communication alert itself cannot be dismissed or suppressed by MiniBrowser.
...
- Delayed isolated-thread recovery also queries Futaba's native search endpoint with the fixed Shift-JIS keyword `定型 ・ ・` every 30 seconds. It merges parent-thread IDs with the direct source-page monitor, validates the smallest eligible successor, and keeps the search page out of the posting WebView.
