...
$isolationMonitorFile = Join-Path $projectRoot 'MiniBrowser\Services\IsolationThreadMonitor.swift'
$searchServiceFile = Join-Path $projectRoot 'MiniBrowser\Services\FutabaThreadSearchService.swift'
...
$networkGateFile = Join-Path $projectRoot 'MiniBrowser\Services\NetworkConnectivityGate.swift'
...
Assert-Contains $projectFile 'sdk:\s*Network\.framework' 'Network framework linkage'
...
Assert-Contains $focusModeFile 'threadAvailabilityScript' 'multi-thread dropped-thread availability probe'
...
Assert-Contains $networkGateFile 'NWPathMonitor' 'local network-path readiness monitor'
Assert-Contains $networkGateFile 'waitUntilSatisfied' 'bounded network-path wait'
Assert-Contains $viewModelFile 'NETWORK_PRECHECK' 'AP network precheck diagnostics'
Assert-Contains $viewModelFile 'NETWORK_POST_AP_GATE' 'post-AP network gate diagnostics'
Assert-Contains $viewModelFile 'automaticAPNetworkWaitNanoseconds' 'bounded AP network wait'
Assert-Contains $listServiceFile 'waitsForConnectivity\s*=\s*true' 'catalog waits for connectivity'
Assert-Contains $isolationMonitorFile 'waitsForConnectivity\s*=\s*true' 'moderation monitor waits for connectivity'
Assert-Contains (Join-Path $projectRoot 'MiniBrowser\Services\IPAddressService.swift') 'waitsForConnectivity\s*=\s*true' 'IP check waits for connectivity'
...
Assert-Contains $listViewFile '隔離・削除時に自動停止' 'moderation-stop checkbox label'
Assert-Contains $viewModelFile 'toggleIsolationStop' 'isolation-stop checkbox action'
Assert-Contains $viewModelFile 'isolationStopEnabled = true' 'isolation-stop default enabled state'
Assert-Contains $isolationMonitorFile 'img_b_isolation\.txt' 'native isolation feed endpoint'
Assert-Contains $isolationMonitorFile 'If-None-Match' 'conditional isolation feed request'
Assert-Contains $viewModelFile 'ISOLATED_THREAD_DETECTED' 'isolation detection diagnostic'
Assert-Contains $automaticFlowFile 'isolatedThread' 'isolation terminal stop reason'
Assert-Contains $viewModelFile 'DELETED_THREAD_DETECTED' 'deleted-thread detection diagnostic'
Assert-Contains $automaticFlowFile 'deletedThread' 'deleted-thread terminal stop reason'
Assert-Contains $isolationMonitorFile 'ModerationThreadSnapshot' 'moderation snapshot model'
Assert-Contains $isolationMonitorFile 'fetchModerationSnapshot' 'moderation feed fetch API'
Assert-Contains $searchServiceFile 'futaba.php\?guid=on' 'native Futaba search endpoint'
Assert-Contains $searchServiceFile '定型 ・ ・' 'fixed next-thread search keyword'
Assert-Contains $searchServiceFile 'mode=search' 'Futaba search form mode'
Assert-Contains $searchServiceFile '\.shiftJIS' 'Shift-JIS search form/response handling'
Assert-Contains $searchServiceFile 'parseSearchResponse' 'native Futaba search response parser'
Assert-Contains $viewModelFile 'SEARCH_REQUEST_STARTED' 'next-thread search diagnostics'
Assert-Contains $viewModelFile 'isolationRecoverySearchIntervalNanoseconds' 'next-thread search interval'
Assert-Contains $listViewFile '隔離・削除時に自動停止' 'moderation stop wording'
Assert-Contains $viewModelFile 'deletedIDs' 'deleted-thread matching'
